...
export class PythonOCREngine {
  ...
  constructor(config: PythonOCRConfig) {
   ...
   * @returns 点选坐标数组
   */
  async recognize(imageBase64: string, promptText: string): Promise<PythonOCRResult[]> {
    ...
    const result: ClickResponse = JSON.parse(responseText);
