...
function _loadCharset(): string[] {
  ...
  for (const p of jsonPaths) {
    if (fs.existsSync(p)) {
      try {
        const charset = JSON.parse(fs.readFileSync(p, 'utf-8')) as string[];
...
interface ClassifyWithConfidenceResult {
  text: string;
  confidence: number;
  topKChars: Set<string>;
}
...
export class ONNXOCRClassifier {
   ...
   * @returns 识别的文字
   */
  async classify(imgBuffer: Buffer): Promise<string> {
   ...
   */
  private _ctcDecodeWithConfidence(outputTensor: ort.Tensor): ClassifyWithConfidenceResult {
    ...
    let seqlen: number;
    let getLogit: (t: number, c: number) => number;
    ...
    const timestepInfo: TimestepInfo[] = new Array(seqlen);
    for (let t = 0; t < seqlen; t++) {
      ...
      for (let c = 0; c < numClasses; c++) {
        ...
        if (topK.length < TOP_K) {
          topK.push({ idx: c, val });
          if (topK.length === TOP_K) topK.sort((a, b) => b.val - a.val);
    ...
    let totalConf = 0;
    let decodedCount = 0;
    const topKChars = new Set<string>();
        ...
        if (idx !== 0) {
          ...
          for (const k of timestepInfo[t].topK) {
            if (k.idx !== idx && k.idx > 0) {
              ...
              if (altChar) topKChars.add(altChar);
            }
          }
        }
      }
