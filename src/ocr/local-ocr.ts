...
import { createCanvas, registerFont, type Canvas, type CanvasRenderingContext2D, type ImageData } from 'canvas';
...
interface OCREnsembleResult {
  char: string;
  confidence: number;
  allResults: Set<string>;
  onnxConfidence: number | null;
}
...
interface DetectedItemWithFeat extends DetectedItem {
  feat: number[];
}
...
// 中文字体白名单（跨平台）
const CHINESE_FONT_PATTERNS: readonly string[] = [
  // Windows
...
export class LocalOCREngine {
  ...
  private variantCache: Record<string, number[][]>;
  fontPaths: FontEntry[];
  private _tesseractWorker: unknown | null;

  private onnxDetector: ONNXDetectionEngine;
  private _onnxReady: boolean;

  private onnxClassifier: ONNXOCRClassifier;
  private _onnxOcrReady: boolean;
   ...
   * 识别点选验证码
   */
  async recognize(imageBase64: string, promptText: string): Promise<OCRResult[] | null> {
    const t0 = Date.now();
    ...
    const t4 = Date.now();
    const detectedWithFeat: DetectedItemWithFeat[] = await Promise.all(detected.map(async (d) => {
    ...
    const variantEntries = await Promise.all(
      [...uniqueChars].map(async (ch): Promise<[string, number[][]]> => [ch, await this._renderVariants(ch)])
    );
    const promptVariants: Record<string, number[][]> = {};
   ...
   */
  private _tryFastPath(detected: DetectedItem[], promptText: string): FastPathMapping[] | null {
    if (!this._onnxOcrReady) return null;

    const n = promptText.length;
    const mapping: FastPathMapping[] = [];
    ...
    for (let pi = 0; pi < n; pi++) {
      ...
      for (let di = 0; di < detected.length; di++) {
        if (usedDetected.has(di)) continue;
  ...
  private async _detectRegions(imgBuffer: Buffer, promptText: string, imgW: number, imgH: number): Promise<DetectedRegion[] | null> {
    ...
    if (this._onnxReady) {
      try {
        ...
        if (!this._onnxReady) {
          try { await this.preInit(); } catch { /* ignore */ }
        }
      }
    }
    ...
    try {
      ...
      const regions = words
        ...
        .map(w => ({ box: [w.bbox.x0, w.bbox.y0, w.bbox.x1, w.bbox.y1] as [number, number, number, number] }));
  ...
  private async _detectByConnectedComponents(imgBuffer: Buffer, imgW: number, imgH: number): Promise<DetectedRegion[] | null> {
      ...
      for (let x = 0; x < width; x++) {
        ...
        if (binary[idx] === 255 && !visited[idx]) {
          ...
          if (component.length >= minArea && component.length <= maxArea) {
            ...
            components.push({ box: [minX, minY, maxX + 1, maxY + 1] as [number, number, number, number], area: component.length });
          }
        }
      }
    }
  ...
  private _mergeCloseComponents(components: ComponentInfo[], mergeDistance: number): ComponentInfo[] {
    ...
    for (let i = 0; i < components.length; i++) {
      ...
      merged.push({ box: [minX, minY, maxX, maxY] as [number, number, number, number], area: totalArea });
    }
  ...
  private async _ocrSingle(imgBuffer: Buffer): Promise<string> {
    try {
      const result = await Tesseract.recognize(imgBuffer, 'chi_sim', {
  ...
  private async _otsuBinarize(imgBuffer: Buffer): Promise<Buffer> {
  ...
  private async _claheEnhance(imgBuffer: Buffer): Promise<Buffer> {
    return sharp(imgBuffer)
      .grayscale()
      .clahe({ width: 4, height: 4, maxSlope: 3 })
      .toBuffer();
  ...
  private async _extractHOGFeature(cropBuffer: Buffer): Promise<number[]> {
    ...
    const globalHog = this._computeHOGBlock(binary, FEAT_SIZE, FEAT_SIZE,
      BLOCK_GLOBAL, STRIDE_GLOBAL, CELL_GLOBAL, NBINS);
    return globalHog;
  }

  private _computeHOGBlock(img: Uint8Array, imgW: number, imgH: number, blockSize: number, blockStride: number, cellSize: number, nbins: number): number[] {
      ...
      for (let cx = 0; cx < cellsX; cx++) {
        ...
        for (let y = cy * cellSize; y < (cy + 1) * cellSize; y++) {
          for (let x = cx * cellSize; x < (cx + 1) * cellSize; x++) {
            if (y < 1 || y >= imgH - 1 || x < 1 || x >= imgW - 1) continue;
   ...
   */
  private async _renderVariants(char: string): Promise<number[][]> {
    ...
    for (const size of sizes) {
      for (const fontPath of topFonts) {
        try {
          const canvas: Canvas = createCanvas(size + 30, size + 30);
          ...
          if (cnWidth < 25 || cnWidth < enWidth * 1.3) continue;
          ...
          for (const angle of angles) {
            ...
            const feat = this._computeHOGBlock(binary, FEAT_SIZE, FEAT_SIZE,
              BLOCK_GLOBAL, STRIDE_GLOBAL, CELL_GLOBAL, NBINS);
            variants.push(feat);
          }
        } catch {
  ...
  private _rotateArray(pixels: Uint8Array, w: number, h: number, angleDeg: number): Uint8Array {
        ...
        for (const file of files) {
          ...
          if (CHINESE_FONT_PATTERNS.some(pat => lowerName.includes(pat.toLowerCase()))) {
            if (lowerName.endsWith('.ttf') || lowerName.endsWith('.ttc') || lowerName.endsWith('.otf')) {
              ...
              try { registerFont(filePath, { family: file }); } catch { /* ignore */ }
            }
          }
        }
      } catch { continue; }
  ...
  private _cosineSim(a: number[], b: number[]): number {
    ...
    na = Math.sqrt(na);
    nb = Math.sqrt(nb);
    if (na < 1e-7 || nb < 1e-7) return 0;
    return dot / (na * nb);
  }

  private _bestVariantSim(variants: number[][], feat: number[]): number {
  ...
  private _permutationSearch(score: number[][], detected: DetectedItemWithFeat[], promptText: string, minDist: number): OCRResult[] {
    const n = promptText.length;
    const m = detected.length;

    function permutations(arr: number[], k: number): number[][] {
      if (k === 0) return [[]];
      const result: number[][] = [];
